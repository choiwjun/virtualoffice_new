...
## S. 자리 주인 지정 = 편집기 안에서, 한 번에 (D38, 2026-07-27)

> D37로 자리를 **놓는** 일은 화면에서 끝나게 됐지만, 그 자리가 **누구 자리인지**는 어디에서도 정할 수
> 없었다. `PUT /api/seat-assignments/{seat_id}`는 구현돼 있었으나 호출하는 화면이 없었고, 해제 경로는
> `POST .../release`(본인 반납)뿐이라 관리자가 남의 자리를 비울 방법이 없었다. 결과적으로 좌석의
> `assigned_user_id`는 시드 데이터 이후 아무도 바꾸지 못하는 값이었다.

| ID | 결정 | 내용 | 폐기·대체 |
|----|------|------|-------------|
| **D38** | 지정과 해제는 **한 엔드포인트** — `PUT /seat-assignments/{seat_id}`의 `user_id`를 **nullable**로 | `user_id`=사원 → 주인 지정, `null` → **주인 없애기**(`status`→available). 해제를 `POST .../release`로 따로 두면 화면이 "지정은 PUT, 해제는 POST"를 분기해야 하고, 그 분기가 곧 부분 실패 지점이 된다. `release`는 **본인 반납** 경로로 남긴다(권한·의미가 다르다 — admin 전용 아님). 이력은 `seat_assignment_history.user_id`가 NOT NULL이라 "주인 없음"을 행으로 쓸 수 없다 → 해제는 **열린 행을 닫기만** 한다. 빈 자리를 또 해제하면 **성공(멱등)** — 두 번 눌러 실패로 보이지 않게. | 해제용 관리자 엔드포인트 신설, `user_id` 필수 |
| **D38-b** | **한 사람 = 한 자리** — 새 자리를 주면 갖고 있던 자리는 서버가 비운다 | `employees.seat_number`가 **단수**라 두 자리를 허용하면 어느 쪽이 보일지가 조회 순서에 좌우된다(같은 사람이 두 자리에 앉아 있는 도면도 나온다). 409로 막아 "먼저 저쪽을 비우세요"를 요구하면 관리자가 2회 호출을 손으로 엮게 되고, 그 사이 실패가 곧 D38이 없애려던 부분 실패다. 화면은 **누르기 전에** 알린다 — `○○님은 지금 A-03에 있습니다. 옮기면 A-03은 빈 자리가 됩니다`. 감사 로그 `new_value.vacated_seat_ids`에 비운 자리를 남긴다. | 두 자리 허용, 409 후 수동 해제 |
| **D38-c** | 대상 사원은 **같은 회사의 활성 사원**만 — 아니면 404 | 없는 사원·퇴사자·타사 사원에게 자리를 넘기면 아무도 쓸 수 없는 **유령 자리**가 되고, 타사 사원 배정은 그 자체가 테넌트 경계 침범이다. 존재 은닉을 위해 400이 아니라 **404**(22 T0-1 레시피). 같은 사원을 다시 지정하면 **무변경**(이력·감사 로그 없음) — 편집기에서 저장을 반복해도 이력이 불어나지 않게. | 사원 검증 없음(FK만 신뢰), 400 반환 |
| **D38-d** | 주인 지정은 **저장 대기 버퍼를 거치지 않는다** | 좌표는 `[저장]`까지 모았다가 적용하지만, 배정은 **DB 소유(D10)** 라 레이아웃 반영과 무관하게 즉시 반영된다. 두 경로를 한 버퍼에 섞으면 "저장 안 했는데 반영된 것"과 "저장했는데 아직 안 된 것"이 한 화면에 공존한다. 대신 화면이 그 차이를 **문장으로 말한다** — `고르는 즉시 반영됩니다. [저장]을 따로 누르지 않아도 됩니다.` 아직 저장 전(temp) 자리는 주인 고르기를 **잠그고 이유를 쓴다**(D37-b). | 좌표와 같은 버퍼에 묶기, 이유 없는 비활성 |
| **D38-e** | `GET /seat-assignments` 테넌트 스코프 보정 | 이력 조회에 회사 필터가 없어 **어느 회사 관리자든 타사 좌석 이력을 통째로 읽을 수 있었다.** `seat_assignment_history`에는 `company_id`가 없으므로 부모(`seat`) 조인으로 스코프한다(22 T0-1 레시피 4). | 무스코프 조회 |

---

## T. 팀 이름의 정본 = 조직도 (D39, 2026-07-27)

> `erp_user`는 팀을 **숫자(`erp_team_id`)로만** 들고 있고 팀 이름을 가진 테이블이 없었다. 그 공백을
> 화면마다 다르게 메웠다 — 직원명부는 프론트에 숫자↔이름 표를 **하드코딩**했고(시드 시절 이름에 멈춰
> 있어 "데이터팀장인데 소속은 디자인팀"으로 읽혔다), 조직도 그래프는 포기하고 `팀 #1`을 찍었다.
> **같은 조직도 화면 안에서** 좌측 트리는 `플랫폼개발팀`, 우측 그래프는 `팀 #1`로 갈렸다.
> 같은 병이 셸에도 있었다 — 오피스 필이 `HORIZON · 판교 HQ`를 박아 두어 어느 테넌트로 로그인해도
> 남의 간판이 걸렸다(E5 화이트라벨 전체가 무의미해진다).

| ID | 결정 | 내용 | 폐기·대체 |
|----|------|------|-------------|
| **D39** | 팀 이름의 정본 = **`org_group.erp_team_id`** (nullable) | 새 `team` 테이블을 만들지 않는다. ERP 없는 회사(E3 native)에서도 조직도는 관리자가 직접 만들므로, 별도 테이블은 그 회사에서 빈 채로 남아 **org_group과 이중 관리**가 된다. 이미 회사별 계층을 들고 있는 `org_group`에 팀 번호를 이으면 이름의 정본이 하나 생긴다 — 관리자가 조직도에서 한 번 이으면 직원명부·그래프·`GET /api/teams`가 전부 같은 이름을 쓴다. `GET /api/teams`는 인원을 `erp_user`에서, 이름·색을 `org_group`에서 얹는다(집계는 그대로 파생). | 프론트 `TEAM_LABELS` 하드코딩, 24-스펙의 별도 team 테이블안, `team_zone`을 이름 경로로 쓰는 방안(그건 구역 매핑이다) |
| **D39-b** | **NULL 허용이 핵심** — 팀이 아닌 계층이 있다 | 본부·파트처럼 사람이 직접 소속되지 않는 노드는 팀이 아니다. NOT NULL로 두면 조직 계층이 곧 팀 목록이 돼 **본부를 만들 수 없다.** 유니크는 `(company_id, erp_team_id)` — 두 그룹이 같은 팀을 주장하면 이름이 조회 순서에 좌우된다. NULL은 SQL 표준상 서로 구별되므로 팀 없는 계층은 얼마든지 공존한다. 회사 스코프가 선행 컬럼이라 번호는 **회사마다 독립**이다(회사1의 팀 1과 회사2의 팀 1은 남남). | 전역 유니크, NOT NULL |
| **D39-c** | **팀 0은 예약** — 그룹이 맡을 수 없다(400) | 0은 E3 유저 생성이 "팀 미배정"에 쓰는 센티널이고 직원명부도 0을 `미배정`으로 읽는다. 그룹이 0을 맡으면 **소속 없는 사람 전원이 그 그룹 이름으로 뭉쳐 보인다.** 미매핑 경고(`UNMAPPED_TEAM`)에서도 0은 제외한다 — 이름 붙일 팀이 아니다. | 0을 일반 팀 번호로 취급 |
| **D39-d** | 이름 없는 팀은 **번호를 그대로 보여 준다** | 서버는 매핑이 없으면 `name: null`을 돌려주고 이름을 지어내지 않는다. 지어내면 화면이 "이름이 없다"와 "이름이 정말 그렇다"를 구별하지 못해 연결 안내를 띄울 수 없다. 화면은 `팀 12`로 폴백하고 그래프는 **점선 테두리**로 표시한다 — 관리자가 이으면 사라지지만, 지어낸 이름은 실제 조직과 어긋난 채 남는다. 미연결은 **오류가 아니라 경고**다(이름 없이도 제품은 돌아간다) → 배포를 막지 않는다. | 서버가 `팀 N` 문자열 생성, 미연결을 배포 차단 오류로 |
| **D39-e** | 연결 해제 센티널 = **`-1`** | PATCH 규약상 `null`은 "이 필드 안 건드림"이라 해제를 표현할 수 없다(이름만 고치는 요청이 매핑을 날리면 안 된다). 음수 팀 번호는 존재하지 않으므로 센티널로 안전하다. | `null` = 해제(다른 필드와 규약 불일치) |
| **D39-f** | 셸 브랜드 표기도 **테넌트 설정에서** | 오피스 필의 회사명·로고를 `useBranding()`으로 바꾼다(로고 없으면 브랜드명 첫 글자 모노그램). 셸이 특정 회사명을 박아 두면 화이트라벨(E5)이 무의미해진다 — TEAM_LABELS와 **같은 병**이다. | `HORIZON · 판교 HQ` 하드코딩, `H` 고정 모노그램 |
| **D39-g** | API 오류의 **구조화 `detail` 통로**를 연다 | 코드만으로는 못 쓰는 안내가 있다 — "팀 1은 이미 **플랫폼개발팀**이 맡고 있습니다"의 그룹명처럼 서버만 아는 값. `detail`이 객체면 `{code, ...맥락}`으로 보고, 프론트 `ApiError.detail`이 그대로 싣는다. 문자열 `detail`은 지금까지대로 `code`로만 오므로 기존 호출부는 무영향. | 문자열 detail만 지원(맥락은 메시지에 문자열로 욱여넣기) |
