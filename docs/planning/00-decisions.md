...
| **D32** (2026-07-05 — 사용자 확정, 오피스 뷰 기본 표현) | 오피스 메인 뷰 = **하이브리드(고품질 렌더 이미지 + 실시간 HTML 오버레이)** | 시안 자체가 "렌더 이미지 + UI 오버레이"임을 확인 → 이를 정본 방식으로 채택. 오피스 중앙은 (a) 고화질 Godot 오프라인 렌더(SDFGI GI·8xMSAA, `frontend/public/office-render.png`, 교체로 업그레이드) 배경 + (b) 사람 이름표·아바타·룸 상태·회의 배지를 **HTML/DOM로 실시간 오버레이**(데이터 변경 즉시 반영). "실시간 3D 입장" 토글로 D31의 Godot WASM 엔진도 유지(양립). 전체 웹 UI/UX는 시안 팔레트(다크 네이비 #0a0e17 + 블루 #3b82f6 + 글래스모피즘)로 재도장. **실시간 웹 엔진(WebGL2)은 GI 부재로 렌더 이미지급 포토리얼 불가** → 히어로 화질은 렌더/데스크톱 Forward+ 담당 | 실시간 엔진만으로 시안 포토리얼 재현(불가), Three.js 재구현 |
