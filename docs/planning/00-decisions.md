...
## U. 배포 레이아웃은 도면, 씬이 사무실 (D40, 2026-07-27~28)

> "회의실을 눌러도 아무 반응이 없다"에서 시작해, **좌표계가 셋으로 갈려 있었다**는 것이 드러났다.
> 씬(V3, 20×11.256m) · 배포 레이아웃(자기 dimensions, 실측 23.8×18.5m) · 이동서버(배포본을 그대로 시뮬레이션).
> 셋 다 좌표를 "미터"라고 불렀지만 크기가 달라 같은 (10, 5)가 서로 다른 자리를 뜻했다.

| ID | 결정 | 내용 | 폐기·대체 |
|----|------|------|-------------|
| **D40** | **화면에 그려진 씬(V3)이 사무실**이고, 배포 레이아웃은 그 위에 얹는 **도면**이다 | 방 상호작용(라벨·글로우·포커스 줌·클릭 히트테스트)과 이동·충돌 지오메트리를 전부 씬에서 가져온다. 코드베이스는 이미 세 곳에서 이 방향을 택했었다 — 30e21bb(배포해도 V3 씬을 항상 렌더) · `applySeatCoords`(좌석을 V3 좌표로 강제) · `V3_ROOMS` 라벨을 `/api/rooms` 이름과 맞춰 둔 것. 그런데 `activeRooms`만 배포본으로 갈아치우고 있어 **배포하면 눈에 보이는 방들의 클릭 대상이 통째로 사라졌다**(배포본 방은 씬에 그려지지도 않는다). | `activeRooms = useDeployed ? dynRooms : v3Rooms`, 보행영역을 배포본 bounds로 잡던 것, 배포본 벽을 충돌로 쓰던 것 |
| **D40-b** | 배포 도면은 **자기 dimensions 기준으로 비율 유지해 스테이지에 맞춘다**(레터박스) | 레이아웃의 `dimensions`는 콘텐츠 외곽+2m로 파생돼(D37) 씬 박스와 아무 관계가 없고 상한도 없다. 씬 상수(`metersToNorm`)로 나누면 씬보다 큰 레이아웃이 정규 1을 넘겨 `overflow-hidden`에 **통째로 잘린다** — 방·벽·구역이 조용히 사라지고, 방 라벨(카드를 여는 유일한 클릭 대상)도 화면 밖으로 나간다. 정규 좌표는 스테이지 박스의 분수라 세로 배율이 가로의 `PLATE_W/PLATE_H`배여야 정사각 회의실이 눌리지 않는다. 씬과 같은 크기를 넣으면 `metersToNorm`과 **수치까지 동일**하다(기존 배포본 회귀 0). | 배포 좌표를 씬 상수로 정규화 |
| **D40-c** | 이동서버의 층과 클라 씬은 **같은 세계여야 하고, 다르면 받지 않는다** | 서버가 배포 레이아웃(23.8×18.5m)을, 클라가 씬(20×11.256m)을 돌리고 있었다. 씬 박스가 배포 박스 안에 들어가 **이동 거부조차 나지 않아** 아무도 모르는 채 좌표만 틀어져 있었다. `HttpFloorLayoutProvider`가 받아온 bounds를 폴백(씬) 층과 대조해 크기가 다르면 씬으로 돌아간다. 더 작은 박스도 거부한다 — 클라가 갈 수 있는 곳을 서버가 전부 막는 2026-07-17 QA와 같은 병이다. 이유는 (office/floor/크기) 조합당 한 번만 로그(refresh 20초마다 반복되면 로그가 의미를 잃는다). | 크기 검증 없이 배포 층 수용, `SCENE_FLOOR=horizon` 기본값(클라 씬 계층이 D35-b에서 제거돼 대응 렌더가 없다) |
| **D40-d** | 씬 방 ↔ DB 회의실의 정본 = **`room.scene_key`** | 카드가 "이 방이 어느 DB 방인가"를 **이름 문자열 일치**로 때우고 있었다. 씬 라벨이 영어(`Board Room`)라 한국어로 이름 붙인 회사는 **한 번도 맞지 않고**(모든 방이 "회의실 정보 없음"), 방 이름을 바꾸면 조용히 끊긴다. 값은 `V3_ROOMS[].id`, NULL은 "씬에 대응 없음"(다른 층·예약 전용), 유니크는 `(company_id, scene_key)`. 씬 방 목록을 백엔드에 복제하지 않고 형식만 강제한다 — 복제하면 씬이 바뀔 때마다 두 곳이 어긋난다. 이름 대조는 **연결 전 데이터용 폴백**으로만 남기되, 이미 다른 씬 방의 주인인 회의실은 후보에서 뺀다. | 이름 문자열 매칭(정본 아님) |
| **D40-e** | 클라 보폭은 **흐른 시간으로 재단**한다 | 고정 `0.07m/50ms`는 정확히 1.4m/s = 서버 상한이라 여유가 0이었다. 서버는 dt를 **도착 간격**으로 재므로 통과 조건이 `dt ≥ 33.3ms`인데 `setInterval`은 간격을 보장하지 않는다(타이머 드리프트·이벤트 루프 밀림·지터) → 간헐 `over_speed` 거부. 보폭을 `min(남은거리, MAX_SPEED·실제 경과)`로 재단하면 정속은 그대로 1.4m/s이면서 예산 안에 들어오고, 지터 여유가 33% 생긴다. | 고정 보폭 |
| **D40-f** | ERP 팀 동기화는 **개편은 따라가고 개명은 따라가지 않는다** | `fetch_teams()`가 구현돼 있는데 아무도 부르지 않아 ERP 연동 회사도 팀 이름을 손으로 이어야 했다. 다만 조직도는 관리자의 것이다 — 우리 화면에서 고친 이름을 다음 동기화가 되돌리면 관리자는 자기가 한 일이 왜 사라지는지 알 수 없다. 그래서 ① 없는 팀은 부서로 신설 ② 이름이 같은 미연결 그룹은 번호만 이어 줌 ③ 이미 누가 맡은 팀은 손대지 않음. 개명까지 따라가려면 `erp_user.source`와 같은 출처 구분을 `org_group`에도 둬야 한다(별도 작업). | ERP 값으로 무조건 덮어쓰기, 동기화가 팀을 무시 |
| **D40-g** | 스키마 드리프트 검사는 **타입·nullability까지** 본다 | 하루에 같은 부류가 두 번 터졌고(`presence.office_id` NOT NULL, `org_group.company_id` CHAR(32)) 둘 다 컬럼 **이름만** 비교하는 기존 검사(`fix_sqlite_drift.py`)를 통과했다. 타입은 선언 문자열이 아니라 **SQLite 친화도**로 비교한다 — `BIGINT` vs `INTEGER`, `VARCHAR(255)` vs `VARCHAR(64)` 같은 무해한 차이를 소음으로 만들면 아무도 안 보게 된다. 드리프트가 있으면 exit 1(게이트). | 컬럼 이름만 대조 |
| **D40-h** | 메일 발송은 **어댑터 뒤**에 둔다 — SMTP/SES 결정이 착수를 막지 않게 | 무엇을 쓸지가 미결이라 E9가 막혀 있었는데, 그 결정은 어댑터 뒤에 있으면 되는 것이다. 표준 라이브러리 SMTP를 기본 구현으로 두고 SES는 `SesMailer` 추가 + 설정 한 줄. 기본값은 **console** — 미설정 환경에서 조용히 "보낸 척"하지 않도록. 발송은 **부가**다: 실패해도 링크는 관리자에게 돌아간다(응답 `mailed`로 구분). 여기서 예외를 올리면 토큰은 발급됐는데 응답이 500이라 관리자는 링크를 잃고 사용자는 메일도 못 받는다. | 발송 채널 결정까지 E9 보류, 발송 실패 시 발급 롤백 |
