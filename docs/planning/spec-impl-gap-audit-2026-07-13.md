...
> **조치 현황 (2026-07-13 2차)**: ① #20 잔여(D17 '18:00 이후 익일 귀속') — `ensure_eod_rows`에 전일 18:00 KST 이후 생성 로그 스윕 구현 + 회귀 테스트. ② #17 — deps.ADMIN_ROLES/MANAGER_ROLES 단일 상수를 9개 API 모듈(kpi·seats·meetings·meeting_minutes·trips·reports·work_logs·chat·consent·directory)에 전면 확산, 인라인 정의 제거 → **#17 종결**. ③ #27 — 08 §6.2.2에 구현 정합 노트(평면 구조 정본화, team_percentile §5.4와 묶어 후속) → 문서 종결. 잔여 = #14(P7 유보)·#21(멀티테넌트 유보)·#27 구조 전환(후속 과제). 검증 336 passed.
...
> **조치 현황 (2026-07-13 2차)**: #1 01-prd **v4.1 개정 완료**(D29 배너 + §2/§3/§7/§8 렌더 서술·수용기준 2.5D 재정의 + chat 범위표 등재). #2 — 14/16(및 07/11)은 **재확인 결과 D29 배너 기부착**(2026-07-12) → 스테일 판정 자체가 과장, 종결. #3 — 15-spec 재확인 결과 8항목에 회의실 정원 기포함(의미 드리프트 없음), 구현 9함수=④ 분리라는 정합 노트만 추가. #4 rbac.yaml 신규 기능 8행 등재 완료. #5 chat 스코프 = 경량 채널 채팅 MVP 선행 + 본격 채팅 Phase 7 유지로 PRD 기록(확장 여부 OQ). 잔여 = #6 12-tasks 표기 드리프트(경미), #7 WA 주석 1줄.
