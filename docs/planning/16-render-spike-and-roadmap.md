...
> 🟦 **D29 피벗(2026-07-12) — 렌더 노선 2.5D로 재대체.** Part A/B의 "Blender 오프라인렌더·깊이합성·실시간 3D glb" 전제는 폐기·보관. 현행 = **2.5D 클린플레이트 PNG + DOM 스프라이트**(`OfficeViewport2D`). P0 깊이합성 스파이크는 이력 보관(미채택). 정본 = **00-decisions §J(D29)**.
