# 핸드오프 — 2026-07-17 세션 (A 3종 + C 설계 → **세션 2: F 전기능 QA 31/31 + C-M0**)
...
| `f7e1af9` | **[세션 2] F 전기능 실화면 QA + realtime 폴백 수리** — dev 3종 스택 기동 후 Playwright 자동 QA(`artifacts/qa/office-qa.cjs`) **31 PASS / 0 FAIL**: 로그인·뷰포트 로딩(스폰 2.1s)·레이어 57장·연결배지·클릭이동·장애물 클램프·내 자리로 sit·typing 버스트 타임라인·테마 3종·미니맵·**D24 보드룸 입장 프롬프트**·2인 동시 상호표시/이동반영·아바타 프리셋 반영·셸 16화면 스모크(스크린샷 31장 = artifacts/qa/shots). **발견·수리**: HttpFloorLayoutProvider 404 폴백이 Demo 고정 → 클라 HORIZON 렌더와 지오메트리 불일치로 전 이동 collision 무한거부 → 폴백을 SCENE_FLOOR 추종으로 수리(layout-smoke 5/5, smoke 30+scene-floor 13 그린) + 런북 갱신. dev_qa.db 픽스처 password_hash 스테일(로그인 401) → 재시드 커밋 | realtime-qa-runbook.md |
| `c964bc2` | **[세션 2] C-M0 모듈 카탈로그** — horizon-scene.js 아이템 57개 meta(type/variant/ob/seat) 전수 태깅 + `extract-modules.js` 신설: **15타입×33변형** 대표 스프라이트(0.25MB, -50%) + catalog.json(instances 57 = M1 scene-layout v2 시드). 게이트 = 재합성 픽셀 diff **0.296% < 1% PASS**, extract-layers 산출물 바이트 불변(무회귀) | 18-문서 §4 M0 |
...
### F. 실모니터 육안 확인 (선택) — 자동 QA는 완료
전 기능 자동 QA 31/31 PASS + 스크린샷 31장(artifacts/qa/shots) 확보. 남은 것은 233Hz 실모니터 취향 확인(야간 테마 톤 강도 등)뿐. dev 스택은 `artifacts/qa/run-{backend,realtime,frontend}.cmd`로 원클릭 기동(런북 §1의 정정판 — SCENE_FLOOR=horizon, LAYOUT_SOURCE_URL 생략).

### C. 모듈 합성 + 좌석 편집기 — **M0 ✅ → M1 착수**
M1 = scene-layout v2 생성기(catalog.json instances가 시드) + 뷰포트 v2 렌더 경로(플래그·3단 폴백). 게이트 = 실화면 z-순서 회귀 0 (office-qa.cjs 재사용). 이후 M2(gen-geometry 코드젠) → M3(편집기) → M4(배치 변형 3종 데이터).
...
- **[세션 2] 기동 함정(수리됨)**: 런북 구판대로 LAYOUT_SOURCE_URL을 주면 배포 레이아웃 404 시 Demo 폴백 → 클라 HORIZON과 지오메트리 불일치 → 스폰 (10,7.5)에서 전 이동 collision 무한거부. 수리 후 폴백은 SCENE_FLOOR 추종. **관찰(미수리)**: 클라 walker가 move_rejected에 같은 타깃을 무한 재전송(백오프/재경로 없음) — 정상 지오메트리에선 발현 안 하나 M1 뷰포트 작업 시 백오프 추가 검토.
- **[세션 2] M0 계약**: 씬 아이템 추가 시 add() 5번째 인자 meta 태깅 필수(미태깅 → extract-modules 실패). 대표 스프라이트 재합성의 서브픽셀 AA 차이가 diff 0.296%의 전부 — 변형 분리 기준은 "내용이 다르면 변형"(머그색·식물 리프 해시 포함).
