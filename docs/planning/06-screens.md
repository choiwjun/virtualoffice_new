...
>
> ⚠ **D29 시안 B 결정 (2026-07-12, 셸 단일화)**: 구현은 뷰포트가 중앙 영역 전체를 차지하는 **시안 B**를 채택 — 3카드 행은 코드에 보존하되 `hidden` 처리(OfficeShell.tsx). 오늘의 업무·KPI 글랜스는 좌내비 도메인 화면으로, 진행중 회의는 뷰포트 우하단 오버레이 카드로 흡수됨. 복원 시 hidden 제거. 본 절의 3카드 명세는 시안 A 참고용으로 유지.
>
> ⚠ **D33 몰입 모드 (2026-07-21, 19-office-ux-simplify-spec 정본)**: **/office 라우트 한정**으로 본 절의 셸 구성이 다음과 같이 덮어써진다 — ① Left Nav 240px → **아이콘 레일 56px 기본**(펼치기 토글, 타 라우트는 240px 유지) ② Right Panel → **접힘 기본**(캔버스 우측 가장자리 "구성원 N" 핸들로 열기) ③ 뷰포트 좌상단 배지 4종·하단 미디어바·우하단 진행중 회의 오버레이 → **하단 중앙 통합 독**(연결 점+툴팁·내 자리로·테마·상태 + LIVE 입장 칩·연결 시 MediaBar) ④ 사이드바 "2F 도면" 위젯 삭제 — 층 전환은 미니맵 헤더로 이동 ⑤ 네임플레이트=이름+상태점만(본인=발밑 파란 링) ⑥ 존 바닥 반투명 색면 + 존 라벨은 호버/클릭 시 표시. 수용 기준·상세는 19-spec G1~G5.
...
| **분기 종합** | `quarterly_total` | 분기 전체 기간 집계 collaboration_score × (기간일수/91) 스케일 — 부분 분기는 비례 축소(분기 확정·ERP push 대상). ~~일일 13주 합산~~ 표현은 2026-07-13 구현 정본으로 정정 | 기간 집계 |
...
- ⚠ **구현 한계 (2026-07-13)**: 현행 2.5D 아바타는 래스터 스프라이트 프레임이라 상의/하의 색이 외형에 직접 반영되지 않음 — top_color는 이름표 테두리·본인 식별 색으로 전용, bottom_color는 예비. 색상의 외형 반영은 스프라이트 리컬러 파이프라인 또는 3D 전환(P7) 시 재개.
...
> §2 좌내비 '커뮤니케이션' 메뉴. ✅ **스코프 확정(D32, 2026-07-17)**: 현 채널 채팅 구현을 **MVP로 승인·정본화**(00-decisions §M). 오픈 직전 동작·검증 완료된 사용자 대면 기능을 비활성화할 실익이 없어 (a)안 채택. **In**: general·team 채널, 불변 메시지(1~2000자), 4초 증분 폴링. **Out(Phase 7 유보, 착수 금지)**: DM·스레드·실시간 WebSocket·파일 업로드·검색·읽음표시. §2의 '회의 메모·근접 DM 한정' 원칙은 채널 메시지까지 허용으로 완화됨.
