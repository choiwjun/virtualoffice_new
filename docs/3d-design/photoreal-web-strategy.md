...
> 🔵 **D28 피벗(2026-07-09) — 이 전략(오프라인렌더+깊이합성)은 폐기·보관.** 실측 결과 사용자 제작 스타일라이즈드 저폴리 에셋으로 **실시간 R3F 단일 렌더**가 3대 난제(깊이합성·유리·아바타 조명매칭)를 원천 제거하며 목표를 충족 → 이 문서의 오프라인렌더/깊이합성/유리 전략은 **채택하지 않는다**. 현행 정본 = **00-decisions §I(D28)**. (문서는 대안 검토 이력으로 보존)
