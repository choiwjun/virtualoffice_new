...
## Git Worktree 및 완료 계약

- Phase 1+ 3D 작업은 오케스트레이터가 전달한 Git Worktree와 Phase 브랜치에서 수행합니다.
- specialist는 구현, 테스트, 회귀 검증, 로컬 커밋까지 완료한 뒤 `TASK_DONE:{task_id}:{commit_sha}`를 보고합니다.
...
WORKTREE_PATH="${WORKTREE_PATH:-$(pwd)/worktree/phase-2-3d}"
PHASE_BRANCH="${PHASE_BRANCH:-phase-2-3d}"
