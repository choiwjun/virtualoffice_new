...
**유일하게 허용되는 확인:** 테스트 요구사항이 모호할 때의 기술 질문만. Phase 병합 여부 질문은 금지.

## 완료 책임

- specialist는 Worktree에서 테스트 설계/수정, TDD 검증, 로컬 커밋까지 완료합니다.
- `main` 병합과 원격 push는 orchestrator만 수행합니다.
- 완료 보고에는 `TASK_DONE:{task_id}:{commit_sha}` 형식을 사용합니다.
...
```
오케스트레이터: "Phase 0, P0-T0.5.1 구현: API 계약 정의"
...
2. 없으면 → git worktree add worktree/phase-1-auth -b phase-1-auth main
...
- specialist는 `TASK_DONE:{task_id}:{commit_sha}` 또는 `FAIL:{task_id}:...`만 보고합니다.
- 사용자에게 main 병합 여부를 묻지 않습니다.
- Phase 병합은 orchestrator가 품질 체인 완료 후 수행합니다.
