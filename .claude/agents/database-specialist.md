...
**유일하게 허용되는 확인:** 스키마/데이터 무결성 관련 기술 질문만. Phase 병합 여부 질문은 금지.

## 완료 책임

- specialist는 Worktree에서 마이그레이션/모델 수정, 검증, 로컬 커밋까지 완료합니다.
...
**Phase 1 작업 요청 시:**
```
오케스트레이터: "Phase 2, P2-R1-T1 구현: users 테이블 스키마"

나의 행동:
1. 전달받은 WORKTREE/BRANCH 확인
2. 없으면 → git worktree add worktree/phase-2-database -b phase-2-database main
3. cd worktree/phase-2-database
4. 해당 경로에서 마이그레이션 구현
```
...
3. **오케스트레이터에 완료 상태만 보고 (필수!)**
   ...
   - 생성된 모델: ...
   - 마이그레이션 결과: alembic upgrade head ✅
   - tests/models 결과: pytest ... ✅
   - commit: <commit-sha>
   ```
