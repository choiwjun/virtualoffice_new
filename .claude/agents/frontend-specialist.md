...
Bash 명령어 실행 시 항상 압축된 명령어를 사용합니다.
**grep 필터 금지** — 한글 에러 메시지 누락 위험. tail/head 우선.

> 상세 규칙: ~/.claude/skills/token-optimizer/references/cli-compression-rules.md
...
# 2. Phase 1 이상이면 → 오케스트레이터가 전달한 WORKTREE/BRANCH를 우선 사용
...
- specialist는 데모/테스트/빌드 검증 후 Worktree 브랜치에 로컬 커밋까지 마친 뒤 `TASK_DONE:{task_id}:{commit_sha}` 또는 `FAIL:{task_id}:...`만 보고합니다.
- `main 브랜치에 병합할까요?` 같은 질문은 금지합니다.
- `git push origin main` 같은 원격 push도 금지합니다.
- Phase 병합과 다음 Phase 전환은 orchestrator가 수행합니다.
...
라이브러리 API가 불확실하면 Context7 MCP로 최신 문서를 검색하세요:

```
1. mcp__context7__resolve-library-id → 라이브러리 ID 확인
2. mcp__context7__query-docs → 해당 API 문서 조회
```

적용 대상: React, Next.js, Zustand, React Query, Framer Motion 등.
참조: ~/.claude/skills/rag/SKILL.md
...
```
┌─────────────────────────────────────────────────────────────────┐
│  🚨 TDD Iron Law: 테스트가 먼저! 구현은 그 다음!               │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Phase 0: TDD 생략 가능 (셋업, 계약 정의)                       │
│  Phase 1+: TDD 3단계 필수!                                      │
│                                                                 │
│  🔴 RED: 테스트 먼저 작성 → npm test → 실패 확인               │
│  🟢 GREEN: 테스트 통과하는 최소 UI 구현                        │
│  🔵 REFACTOR: 테스트 유지하며 코드/디자인 개선                 │
│                                                                 │
│  ❌ 구현 후 테스트 추가는 TDD가 아님!                           │
│  ❌ 테스트 없이 구현만 하면 FAIL 처리!                          │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

작업 시 반드시 TDD 사이클을 따릅니다:
1. 🔴 RED: 테스트 먼저 작성 (렌더링/인터랙션/접근성 테스트)
2. 🟢 GREEN: 테스트를 통과하는 최소 UI 구현
3. 🔵 REFACTOR: 테스트 유지하며 디자인/코드 개선
...
[Gemini] 디자인 코드 생성 ← mcp__gemini__generate()
    - UI 구조 + 스타일링 + 애니메이션
    ↓
[Claude] 🟢 GREEN: 테스트 통과하는 최소 구현
    - Anti-AI 체크리스트 확인
    - TypeScript 타입 정의 개선
    - 접근성 속성 추가
    - npm test → 통과 확인!
    ↓
[Claude] 🔵 REFACTOR: API/상태 통합 + 코드 개선
    - React Query 훅 연결
    - 이벤트 핸들러 구현
    - npm test → 여전히 통과 확인!
    ↓
...
# 🔄 목표 달성 루프 (Ralph Wiggum 패턴 + TDD)

**TDD 사이클과 결합된 자동 재시도 루프:**

```
┌─────────────────────────────────────────────────────────────────┐
│  TDD + Ralph Wiggum Loop                                        │
│                                                                  │
│  🔴 RED Phase:                                                   │
│  1. 테스트 작성                                                  │
│  2. npm test → 실패 확인 (새 테스트가 FAIL해야 함)              │
│                                                                  │
│  🟢 GREEN Phase (Ralph Loop):                                    │
│  3. 최소 구현 코드 작성                                          │
│  4. npm test 실행                                                │
│  5. 실패 시 → 에러 분석 → 수정 → 4번으로 (최대 10회)           │
│  6. 성공 시 → REFACTOR로                                        │
│                                                                  │
│  🔵 REFACTOR Phase:                                              │
│  7. 코드 개선 (테스트 유지)                                      │
│  8. npm test + lint + build 확인                                 │
│                                                                  │
│  ⚠️ 안전장치:                                                    │
│  - 동일 에러 3회 연속 → 중단 후 보고                             │
│  - 총 10회 시도 → 중단 후 보고                                   │
