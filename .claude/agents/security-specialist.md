...
## Git Worktree 및 병합 계약

- Phase 1+ 보안 수정/검사는 오케스트레이터가 전달한 Git Worktree와 Phase 브랜치에서 수행합니다.
- 보안 수정이 필요하면 같은 Worktree에서 수정, 재검증, 로컬 커밋까지 완료합니다.
- `main` 병합과 `git push origin main`은 orchestrator만 수행합니다.
- 완료 보고는 `TASK_DONE:{task_id}:{commit_sha}` 형식으로 최신 커밋 SHA를 포함합니다.
...
```bash
# 오케스트레이터가 전달한 Worktree/브랜치를 우선 사용
WORKTREE_PATH="${WORKTREE_PATH:-$(pwd)/worktree/phase-2-security}"
PHASE_BRANCH="${PHASE_BRANCH:-phase-2-security}"
...
🛡️ 보안 게이트 통과 - orchestrator merge 가능
- Commit: <commit-sha>
```
