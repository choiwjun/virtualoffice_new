...
**유일하게 허용되는 확인:** 구현 범위/요구사항이 모호할 때의 기술 질문만. Phase 병합 여부 질문은 금지.

## 완료 책임

- specialist는 Worktree에서 구현, TDD, 검증, 로컬 커밋까지 완료합니다.
- `main` 병합과 `git push origin main`은 orchestrator만 수행합니다.
- 완료 보고에는 반드시 `TASK_DONE:{task_id}:{commit_sha}` 형식의 최신 커밋 SHA를 포함합니다.
...
적용 대상: FastAPI, SQLAlchemy, Pydantic, Alembic 등 주요 의존성.
...
```bash
# 1. Worktree 존재 여부 확인
git worktree list | grep "worktree/phase-{N}-{feature}" || git worktree add worktree/phase-{N}-{feature} -b phase-{N}-{feature} main
...
**Phase 1 작업 요청 시:**
```
오케스트레이터: "Phase 2, P2-R1-T1 구현: /api/auth/login"

나의 행동:
1. 전달받은 WORKTREE/BRANCH 확인
2. 없으면 → git worktree add worktree/phase-2-resources -b phase-2-resources main
3. cd worktree/phase-2-resources
4. 해당 경로에서 코드 구현
```

**Phase 0 작업 요청 시:**
```
오케스트레이터: "Phase 0, P0-T0.1 구현: 프로젝트 셋업"
...
**⚠️ Phase 병합은 오케스트레이터가 수행합니다. specialist는 사용자에게 병합 여부를 묻지 않습니다.**
