...
const ICON = {
  ...
  upload: <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth={1.7} strokeLinecap="round" strokeLinejoin="round" className="w-full h-full"><path d="M10 13V4" /><path d="M6.5 7.5 10 4l3.5 3.5" /><path d="M4 16h12" /></svg>,
  spark: <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth={1.7} strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4"><path d="M10 3v3M10 14v3M3 10h3M14 10h3" /><path d="M6 6l2 2M14 6l-2 2M6 14l2-2M14 14l-2-2" /></svg>,
  trophy: <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth={1.7} strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4"><path d="M6 4h8v3a4 4 0 0 1-8 0z" /><path d="M6 5H4v1a2 2 0 0 0 2 2M14 5h2v1a2 2 0 0 1-2 2M8 12h4M10 12v3M8 16h4" /></svg>,
};
  ...
  return (
    <div className="space-y-2">
      {deltaPct != null && Math.abs(deltaPct) >= 30 && (
        <div className="px-3 py-2 bg-[rgba(245,158,11,0.16)] border border-[rgba(245,158,11,0.4)] rounded-md text-xs text-status-external">
          ⚠️ 이전 분기 대비 {deltaPct > 0 ? '+' : ''}
          {deltaPct.toFixed(0)}% 급변 — 조정 전 원인 확인을 권장합니다.
        </div>
      )}
      {trend.length > 0 && (
        <div className="bg-bg-base rounded-md px-3 py-2">
          <div className="text-[11px] text-text-muted mb-1">최근 분기 추이</div>
          <div className="flex items-end gap-2">
            {trend.map((t) => (
              <div key={t.period} className="flex-1 flex flex-col items-center min-w-0">
                <span className="text-[10px] text-text-secondary leading-none mb-0.5">
                  {formatScore(t.score)}
                </span>
                ...
                <span className="mt-0.5 text-[10px] text-text-muted">{t.period.slice(2)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {teamAvg != null && (
        <div className="flex items-center justify-between bg-bg-base rounded-md px-3 py-2 text-xs">
          <span className="text-text-muted">팀 평균 (동일 지표·기간)</span>
          <span className="font-semibold text-text-secondary">{teamAvg.toFixed(1)}</span>
        </div>
      )}
    </div>
  );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div role="dialog" aria-modal="true" aria-label={p.title} className="rounded-xl shadow-2xl w-full max-w-md border border-border-subtle" style={{ background: 'rgb(var(--color-bg-surface))' }}>
        ...
        <div className="px-6 py-4 space-y-4 max-h-[80vh] overflow-y-auto">
          ...
          <div className="flex items-center justify-between bg-bg-base rounded-md px-3 py-2 text-sm">
            <span className="text-text-muted">원점수</span>
            <span className="font-semibold text-text-primary">{formatScore(p.origin)}</span>
          </div>
          {p.extra}
          <div>
            <label className="block text-sm font-medium text-text-secondary mb-1">{p.scoreLabel}</label>
            <input
              type="number"
              step="0.1"
              min={lo}
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <label className="block text-sm font-medium text-text-secondary">{p.noteLabel}</label>
              {p.noteMinLen > 0 && (
                <span className={`text-xs ${noteValid ? 'text-text-muted' : 'text-red-300'}`}>
...
export default function AdminKpiPage() {
  ...
  if (!allowed) {
    return (
      <div className="p-6 text-text-secondary">
        <div className={`${CARD_SURFACE} max-w-md mx-auto mt-20 p-8`}>
          <EmptyState
            icon="🔒"
            title="관리자 전용 화면"
            hint="KPI 관리는 관리자/리더만 접근할 수 있습니다."
          />
        </div>
      </div>
    );
  ...
  return (
    <div className="p-6 flex flex-col gap-5 h-full text-text-secondary max-w-6xl mx-auto">
      <PageHeader
        title="KPI 관리"
        subtitle="대상자 KPI 계산·조정·확정 (D16 관리자 라우팅)"
        icon={ICON.gauge}
      />

      <div className={`${CARD_SURFACE} p-4 flex flex-wrap items-end gap-3`}>
        <label className="flex flex-col text-xs text-text-muted gap-1">
          대상 직원
          <select
            value={targetId ?? ''}
            onChange={(e) => setTargetId(Number(e.target.value))}
            className="border border-border-subtle bg-bg-base rounded-md px-2 py-1.5 text-sm text-text-primary min-w-48 focus:outline-none focus:ring-2 focus:ring-accent-cyan"
          >
            {employees.map((emp) => (
        ...
        <ToolbarButton onClick={fetchResults} icon={ICON.refresh}>
          조회
        </ToolbarButton>
        <ToolbarButton onClick={runCompute} disabled={busy === 'compute'} variant="primary" icon={ICON.play}>
          {busy === 'compute' ? '계산 중...' : 'KPI 계산 실행'}
        </ToolbarButton>
        <ToolbarButton onClick={exportErp} disabled={pushing} icon={ICON.upload}>
      ...
      {warn && (
        <div className="px-3 py-2 bg-[rgba(245,158,11,0.16)] border border-[rgba(245,158,11,0.4)] rounded-md text-sm text-status-external">{warn}</div>
      )}
      ...
      <Segmented
        ...
        onChange={setTab}
        options={[
          { value: 'detail', label: '지표 상세' },
          { value: 'ranking', label: '팀 랭킹' },
        ]}
      />
      ...
      {tab === 'detail' && periodType === 'quarterly' && sorted.length > 0 && (() => {
        ...
        return (
          <SectionCard title="AI 평가 초안" icon={ICON.spark}>
            {withDraft ? (
              <AiDraftView draft={withDraft.ai_draft} />
            ) : (
              <p className="text-xs text-text-muted">AI 초안이 아직 없습니다. 21:00 야간 배치(D17)에서 생성됩니다.</p>
            )}
          </SectionCard>
        );
      })()}
      ...
      {tab === 'ranking' && (
        <SectionCard title="팀 랭킹 (종합점수)" icon={ICON.trophy}>
          {rankLoading ? (
            <LoadingState />
          ) : ranking.length === 0 ? (
            <EmptyState icon="🏆" title="팀 데이터가 없습니다." compact />
          ) : (
            <ol className="space-y-1">
              {ranking.map((m, i) => (
                <li key={m.email} className="flex items-center justify-between text-sm py-1 border-b border-border-subtle last:border-0">
                  <span className="flex items-center gap-2">
                    <span className={`w-5 text-center text-xs font-bold ${i < 3 ? 'text-accent-cyan' : 'text-text-muted'}`}>{i + 1}</span>
                    <span className="text-text-primary">{m.name}</span>
                    <span className="text-xs text-text-muted">{m.email}</span>
                  </span>
                  <span className="font-semibold text-text-primary">{m.score != null ? m.score.toFixed(1) : '—'}</span>
                </li>
              ))}
            </ol>
          )}
      ...
      {tab === 'detail' && (loading ? (
        ...
        <EmptyState icon="🧮" title="결과가 없습니다." hint={'"KPI 계산 실행"으로 생성하세요.'} />
      ) : (
        <div className={`${CARD_SURFACE} overflow-hidden`}>
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border-subtle">
              {sorted.map((r) => (
                <tr key={r.id} className={r.metric === 'quarterly_total' ? 'bg-[rgba(59,91,254,0.12)]' : ''}>
                  <td className="px-4 py-2.5 text-text-primary">{metricLabel(r.metric)}</td>
                  <td className="px-4 py-2.5 text-right text-text-secondary">{formatScore(r.value)}</td>
                  <td className="px-4 py-2.5 text-right text-text-secondary">{formatScore(r.admin_adjusted_score)}</td>
                  <td className="px-4 py-2.5 text-right font-semibold text-text-primary">{formatScore(r.final_score)}</td>
                  <td className="px-4 py-2.5 w-40">
                    {(() => {
                      const pct = Math.max(0, Math.min(100, r.final_score ?? r.value ?? 0));
                      const color = pct >= 80 ? 'bg-status-online' : pct >= 50 ? 'bg-status-external' : 'bg-red-400';
                      return (
                        <div className="h-2 w-full bg-bg-surface-raised rounded-full overflow-hidden">
                  ...
                  <td className="px-4 py-2.5 text-center whitespace-nowrap">
                    {r.finalized_at ? (
                      <span className="text-xs text-status-online" title={formatKst(r.finalized_at)}>확정</span>
                    ) : (
                      <span className="text-xs text-text-muted">미확정</span>
                    )}
                    ...
                    {r.pushed_to_erp && (
                      <span className="ml-1 text-[10px] px-1 py-0.5 rounded bg-[rgba(56,189,248,0.15)] text-accent-cyan" title={formatKst(r.pushed_at)}>ERP↑</span>
                    )}
                  </td>
                  <td className="px-4 py-2.5 text-right whitespace-nowrap">
                    {r.objection_status === 'submitted' && (
                      <button
                        ...
                        disabled={busy === r.id}
                        className="text-xs px-2 py-1 border border-[rgba(56,189,248,0.4)] text-accent-cyan rounded hover:bg-[rgba(56,189,248,0.12)] disabled:opacity-40 mr-1"
                      >
                        검토 시작
                      </button>
                    )}
                    {r.objection_status === 'reviewing' && (
                      <button
                        ...
                        disabled={busy === r.id}
                        className="text-xs px-2 py-1 bg-status-online text-white rounded hover:bg-[rgba(34,197,94,0.85)] disabled:opacity-40 mr-1"
                      >
                        이의 처리
                      </button>
                    )}
                    <button
                      ...
                      className="text-xs px-2 py-1 border border-border-subtle rounded text-text-secondary hover:bg-bg-surface-raised disabled:opacity-40 mr-1"
                    >
                      조정
                    </button>
                    <button
                      onClick={() => finalize(r)}
                      disabled={busy === r.id || !!r.finalized_at}
                      className="text-xs px-2 py-1 bg-primary text-white rounded hover:bg-primary-hover disabled:opacity-40"
                    >
                      확정
                    </button>
                  </td>
